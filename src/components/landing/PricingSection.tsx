...
const platformItems = [{
  service: 'Oprava Android zariadení',
  price: 'Na dopyt',
  description: 'Samsung, Xiaomi, Huawei, OnePlus a ďalšie Android telefóny a tablety',
  icon: TabletSmartphone,
  platforms: ['Android']
}, {
  service: 'Oprava Apple zariadení',
  price: 'Na dopyt',
  description: 'iPhone, iPad, MacBook, iMac a ďalšie Apple produkty',
  icon: TabletSmartphone,
  platforms: ['Apple']
}, {
  service: 'Oprava Windows notebookov',
  price: 'Na dopyt',
  description: 'Dell, HP, Lenovo, ASUS, Acer a ďalšie Windows zariadenia',
  icon: Laptop,
  platforms: ['Windows']
}, {
  service: 'Oprava macOS zariadení',
  price: 'Na dopyt',
  description: 'MacBook Air, MacBook Pro, iMac, Mac Mini a Mac Studio',
  icon: Laptop,
  platforms: ['macOS']
}];
export default function PricingSection() {
  return <section id="cennik" className="relative py-24 md:py-32 bg-slate-50 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="space-y-4">
            {pricingItems.map((item, index) => <motion.div key={item.service} initial={{
            opacity: 0,
            x: -20
          }} whileInView={{
            opacity: 1,
            ...
            duration: 0.5,
            delay: index * 0.1
          }} className={`group relative p-6 rounded-2xl border transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 ${item.popular ? 'bg-gradient-to-r from-blue-50 to-blue-100/50 border-blue-200 shadow-md' : 'bg-background border-slate-200 hover:border-blue-200'}`}>
                {item.popular && <div className="absolute -top-3 left-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  <div className={`shrink-0 w-14 h-14 rounded-2xl flex items-center justify-center ${item.popular ? 'bg-blue-100' : 'bg-slate-100 group-hover:bg-blue-50'} transition-colors`}>
                    <item.icon className={`w-7 h-7 ${item.popular ? 'text-blue-700' : 'text-slate-600 group-hover:text-blue-700'} transition-colors`} />
                  </div>
          ...
          delay: 0.3
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {platformItems.map((item, index) => <motion.div key={item.service} initial={{
              opacity: 0,
              y: 10
            }} whileInView={{
              opacity: 1,
          ...
          delay: 0.4
            ...
            <Link to="/price" onClick={() => window.scrollTo({
            top: 0,
            behavior: 'smooth'
          })}>
    ...
    delay: 0.6
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="grid sm:grid-cols-2 gap-5">
          <div>
            <Input type="text" placeholder="Vaše meno" value={formData.name} onChange={e => setFormData({
            ...formData,
            name: e.target.value
          })} required className="h-12 px-4 rounded-xl border-slate-200 focus:border-blue-500 focus:ring-blue-500" />
          </div>
          <div>
        ...
        <div>
          <Input type="tel" placeholder="Telefónne číslo" value={formData.phone} onChange={e => setFormData({
          ...formData,
        ...
        <div>
          <Textarea placeholder="Opíšte problém s vaším zariadením (napr. 'iPhone 12 sa nezapína po páde', 'Notebook sa prehrievá a vypína')" value={formData.description} onChange={e => setFormData({
          ...formData,
          description: e.target.value
        })} required rows={5} className="px-4 py-3 rounded-xl border-slate-200 focus:border-blue-500 focus:ring-blue-500 resize-none" />
        </div>

        <Button type="submit" disabled={isSubmitting} className="w-full bg-blue-700 hover:bg-blue-800 text-primary-foreground font-semibold py-6 rounded-2xl text-lg shadow-lg shadow-blue-700/20 transition-all hover:shadow-xl hover:shadow-blue-700/30">
